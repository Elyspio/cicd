...
const localStorages = {
	validation: container.get<LocalStorageService>(
		DiKeysService.localStorage.validation,
	),
	settings: container.get<LocalStorageService>(
		DiKeysService.localStorage.settings,
	),
};
...
function waitForLogin(page: Window) {
	return new Promise<void>(async (resolve) => {
		...
		const func = () => {
			...
			let isPresent =
				localStorages.validation.retrieve(undefined) !== undefined;
			if (isPresent) {
...
export const login = createAsyncThunk(
	...
	async (_, { getState, dispatch }) => {
		const { logged, username, settings } = (getState() as StoreState)
			.authentication;
		if (!logged || username === undefined) {
			...
			if (page != null) {
				...
				throw new Error(
					"An error occurred while opening the login page",
				);
			}
		} else {
